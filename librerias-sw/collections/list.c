...
void list_remove_all_by_condition(t_list * self, bool (*condition)(void *)) {
	int cantidadDeValores = list_count_satisfying(self, condition);
	int i;
	for (i = 0; i < cantidadDeValores; i++) {
		list_remove_by_condition(self, condition);
	}
